<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  <div class="container">
    <a class="navbar-brand" href="index.php?action=home">Sistema de Clientes</a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>

    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav mr-auto"> <!-- 'mr-auto' empuja los elementos a la izquierda -->
        <li class="nav-item active">
          <a class="nav-link" href="index.php?action=home">Inicio</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="index.php?action=create">Crear </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="index.php?action=list">Consultar</a>
        </li>

        <li class="nav-item">
          <a class="nav-link" href="index.php?action=list_nodos">Nodos</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="index.php?action=create_nodo">Crear Nodo</a>
        </li>

        <li class="nav-item">
          <a class="nav-link" href="index.php?action=list_clientes_nodo">ClientNodos</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="index.php?action=create_cliente_nodo">Crear clientNodo</a>
        </li>

        <li class="nav-item">
          <a class="nav-link" href="index.php?action=logout">Salir</a>
        </li>
        
      </ul>
    </div>
  </div>
</nav>
